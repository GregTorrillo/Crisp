...
const BottomSheetDetails = (props) => {
    const {totalMiles, totalMinutes, onAccepted } = props;
    const isDriverClose = totalMiles <= 1; //decrease for higher accuracy
        ...
        <View style={styles.deliveryDetailsContainer}>
          ...
          <View style={styles.orderDetailsContainer}>
            {dishes?.map((dishItem) => (
              ...
              {dishItem.Dish.name} x{dishItem.quantity} </Text>
            ))}
          </View>
          <Pressable
            style={{
